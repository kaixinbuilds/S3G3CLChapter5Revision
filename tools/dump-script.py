...
w("### 开场（输入姓名与班级）")
w("")
w("- 标题：**⛰️ 学海登顶**；副题：单元五《学无止境》· 三关共 40 分")
...
w("")
w("成绩单显示：姓名、班级、三关分数与用时、总分 /40、总用时、完成日期与时间、验证码。")
w("")
w("---")
w("")
...
for li, lv in enumerate(LEVELS):
    mx = lv["max"]
    w("## %s、%s · %s（%d 分）" % (CN[li], lv["n"], lv["t"], mx))
    w("")
    for pi, d in enumerate(lv["panels"]):
        kind = d.get("k")
        pmax = len([c for c in d["cards"] if c["s"] >= 0]) if kind == "slots" else 1
        w("### 版面 %d · %s（%d 分）" % (pi + 1, d["hd"], pmax))
        w("")
        w("**指示（华文）**：" + B(d["tip"]["zh"]))
        w("")
...
w("| 出处 | 事项 |")
w("|---|---|")
w("| 第二关 版面 3 · 反问句 | 「人生就像一场盛大的演出，我们如何在这场演出中找准自己的位置呢？」"
  "判为**不是**反问句。此句为真疑问，是本版面唯一的否定项，也是鉴别点。"
  "「如何……呢」易被误判，建议另找同事复核反问／设问／疑问的界线。 |")
w("| 第二关 版面 5 · 修辞手法 | 旁批只问「什么修辞手法」，课本未给答案。已依审阅决定改为**多选**"
  "（排比＋比喻），避开二选一的争议。若要改回单选，把该版面的 `k:'multi'` 改成 `k:'choice'`。 |")
w("| 第二关 版面 2 · 三条引语 | 「意思」为拟答（课本留白），已照审阅稿写入，请定稿。 |")
w("| 第三关 版面 5 · 俗语 | 另两个选项由建置时拟定，请审："
  "②「顺水行船…不必费力」（误读「逆」）③「遇到逆流就该停下来等待」（误读「不进则退」）。 |")
w("| 第三关 版面 4 · 篇章结构 | 第4段与第5段合并为一张卡，请确认可行。 |")
w("")
